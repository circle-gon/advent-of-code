...
function sort(mem) {
  // Prevent big endianness issues
  const view = new DataView(mem.buffer);
  const size = view.getInt32(0, true) / 4 - 1;
  const arr = new Int32Array(size);

  for (let i = 0; i < size; i++) {
    arr[i] = view.getInt32(4 * (i + 1), true);
  }

  arr.sort((a, b) => a - b);

  for (let i = 0; i < size; i++) {
    view.setInt32(4 * (i + 1), arr[i], true);
  }
}

// Workaround that sort() needs a reference to the module itself
let m;

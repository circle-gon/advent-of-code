...
async function part1interactive(module, update) {
  let promise, resolve;
  ...
  prompt.addEventListener("keydown", (e) => {
    if (e.key === "Enter") resolve(prompt.value);
  });
  go.addEventListener("click", () => resolve(prompt.value));
  update([
  ...
  while (true) {
    ...
    output.textContent = new TextDecoder().decode(
      new Uint8Array(module.output.buffer).subarray(0, module.outIdx.value),
    );
    prompt.value = "";
    ({ promise, resolve } = Promise.withResolvers());
    memstr((await promise) + "\n", module.commands);
    if (res) return;
  }
}

async function part1(input, update, _, interactive) {
  const { module, memory } = await compilee;
  memstr(input, memory);
  module.part1();
  return interactive ? part1interactive(module, update) : module.part1boring();
}
